...
# ----------------------- Configuration & Setup -----------------------
print("Make sure input file have columns:  company_name,domain,keyword,company_url,context(opt)")

# Input_fie Name Enter file path __.csv
load = "Temp"
# Output_file Name
...
CHECKPOINT_FILE = f"result_io/{csv_name}_io_result.csv"
JSON_CHECKPOINT_FILE = f"json/result_io_json/{csv_name}_io_result.json"

# ------------------ File Header -------------------------------------
# HEADERS = ["Company Name", "Domain", "Page URL", "Keyword", "Date", "Context", "Usage Indicated", "Explanation","Processing Time (s)"]
CSV_SUMMARY_HEADERS = ["Company ID,Company Name", "Domain", "Page URL", "Keyword", "Date", "Usage Indicated", "Explanation"]
...
def main():
    ...
    try:
        ...
        # required_columns = ['Company_ID','company_name', 'company_url', 'keyword', 'domain_name','context']
        required_columns = ['company_name','domain', 'company_url', 'keyword']
    ...
    for index, row in df_input.iterrows():
        ...
        print(f"| {index+1}. Processing URL: {current_url}, Keyword: {keyword}, Company: {comp_name}")
        ...
        try:
            if provided_context is not None:
                # Use the provided context from input file
            ...
            result = {
                ...
                "Context": contexts if contexts else "No context found",
